...
using DocHelper.Domain.Extensions;

namespace DocHelper.Domain.Cache.Generator
{
    ...
    internal class DeepCloneState
    {
        ...
        private class MiniDictionary
        {
            ...
            private MiniDictionary(int capacity)
            {
                if (capacity > 0)
                    Initialize(capacity);
            }
    ...
    {
        private static readonly ConcurrentDictionary<Type, object> TypeCache = new ConcurrentDictionary<Type, object>();

        private static readonly ConcurrentDictionary<Type, object> TypeCacheDeepTo = new ConcurrentDictionary<Type, object>();

        private static readonly ConcurrentDictionary<Type, object> TypeCacheShallowTo = new ConcurrentDictionary<Type, object>();

        private static readonly ConcurrentDictionary<Type, object> StructAsObjectCache = new ConcurrentDictionary<Type, object>();

        private static readonly ConcurrentDictionary<Tuple<Type, Type>, object> TypeConvertCache = new ConcurrentDictionary<Tuple<Type, Type>, object>();
        ...
        public static object GetOrAddClass<T>(Type type, Func<Type, T> adder)
        {
            ...
            object value;
            if (TypeCache.TryGetValue(type, out value)) return value;
            value = adder(type);
        ...
        {
            object value;
            if (TypeCacheDeepTo.TryGetValue(type, out value)) return value;
            value = adder(type);
            TypeCacheDeepTo.TryAdd(type, value);
            return value;
        }
        ...
        {
            object value;
            if (TypeCacheShallowTo.TryGetValue(type, out value)) return value;
            value = adder(type);
            TypeCacheShallowTo.TryAdd(type, value);
            return value;
        }
        ...
        public static object GetOrAddStructAsObject<T>(Type type, Func<Type, T> adder)
        {
            ...
            // this implementation is slightly faster than getoradd
            object value;
            if (StructAsObjectCache.TryGetValue(type, out value)) return value;
            value = adder(type);
            StructAsObjectCache.TryAdd(type, value);
            return value;
        }

        public static T GetOrAddConvertor<T>(Type from, Type to, Func<Type, Type, T> adder)
        {
            return (T)TypeConvertCache.GetOrAdd(new Tuple<Type, Type>(from, to), (tuple) => adder(tuple.Item1, tuple.Item2));
        }
    ...
    internal static class DeepClonerExprGenerator
    {
        ...
        private static object GenerateProcessArrayMethod(Type type)
        {
            ...
            if (elementType is not null && (rank != 1 || type != elementType.MakeArrayType()))
            {
	...
	internal static class DeepClonerSafeTypes
    {
        ...
        private static bool CanReturnSameType(Type type, HashSet<Type> processingTypes)
        {
			...
			// do not copy db null
			if (type.FullName != null && type.FullName.StartsWith("System.DBNull"))
			{
				KnownTypes.TryAdd(type, true);
				return true;
			}

			if (type.FullName != null && type.FullName.StartsWith("System.RuntimeType"))
			{
				KnownTypes.TryAdd(type, true);
				return true;
			}
			
			if (type.FullName != null && type.FullName.StartsWith("System.Reflection.") && Equals(type.GetTypeInfo().Assembly, typeof(PropertyInfo).GetTypeInfo().Assembly))
			{
				KnownTypes.TryAdd(type, true);
				return true;
			}
			...
			if (type.FullName != null && type.FullName.StartsWith("Microsoft.Extensions.DependencyInjection."))
			{
				KnownTypes.TryAdd(type, true);
				return true;
			}
